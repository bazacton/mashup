...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu {
	...
	if (!empty($item->_invalid)) {
	    ...
	    $title = sprintf(__('%s (Invalid)', 'mashup'), $item->title);
	} elseif (isset($item->post_status) && 'draft' == $item->post_status) {
	    $classes[] = 'pending';
	    /* translators: %s: title of menu item in draft status */
	    $title = sprintf(__('%s (Pending)', 'mashup'), $item->title);
	}
	...
	<li id="menu-item-<?php echo absint($item_id); ?>" class="<?php echo implode(' ', $classes); ?>">
		...
		<div class="menu-item-handle">
			...
			<span class="item-order hide-if-js">
			    ...
			    ?>" class="item-move-up" aria-label="<?php esc_attr_e('Move up', 'mashup'); ?>">&#8593;</a>
			    |
			    <a href="<?php
			    echo wp_nonce_url(
				    add_query_arg(
			    ...
			    ?>" class="item-move-down" aria-label="<?php esc_attr_e('Move down', 'mashup'); ?>">&#8595;</a>
			</span>
			<a class="item-edit" id="edit-<?php echo absint($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'mashup'); ?>" href="<?php
			echo ( isset($_GET['edit-menu-item']) && $item_id == $_GET['edit-menu-item'] ) ? admin_url('nav-menus.php') : add_query_arg('edit-menu-item', $item_id, remove_query_arg($removed_args, admin_url('nav-menus.php#menu-item-settings-' . $item_id)));
			?>"><?php esc_html_e('Edit Menu Item', 'mashup'); ?></a>
		    </span>
		</div>
	    </div>
		...
		<p class="description description-thin">
			...
			<?php esc_html_e('Navigation Label', 'mashup'); ?><br />
		...
		<p class="description description-thin">
			...
			<?php esc_html_e('Title Attribute', 'mashup'); ?><br />
		...
		<p class="field-link-target description">
			...
			<?php esc_html_e('Open link in a new tab', 'mashup'); ?>
		    </label>
		</p>
		<p class="field-css-classes description description-thin">
		    <label for="edit-menu-item-classes-<?php echo absint($item_id); ?>">
			<?php esc_html_e('CSS Classes (optional)', 'mashup'); ?><br />
		...
		<p class="field-xfn description description-thin">
			...
			<?php esc_html_e('Link Relationship (XFN)', 'mashup'); ?><br />
		...
		<p class="field-description description description-wide">
			...
			<?php esc_html_e('Description', 'mashup'); ?><br />
			...
			<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.', 'mashup'); ?></span>
		    </label>
		</p>        
		<?php
		...
		<p class="field-custom description description-wide custom_onof">
			...
			<?php esc_html_e('Active Mega Menu', 'mashup'); ?><br />
			...
			if (esc_attr($item->megamenu) == 'on') {
			    echo esc_html__('checked="checked"' , 'mashup');
			}
			?>>
		...
		<p class="field-view description description-wide">
			...
			<?php esc_html_e('Mega Menu View', 'mashup'); ?><br />
			<select id="edit-menu-item-view-<?php echo absint($item_id); ?>" onchange="mashup_menu_view_select(this.value, '<?php echo absint($item_id); ?>')" class="widefat edit-menu-item-view" name="menu-item-view[<?php echo absint($item_id); ?>]">
			    <option<?php echo esc_attr($item->view) == 'simple' ? ' selected="selected"' : '' ?> value="simple"><?php esc_html_e('Simple', 'mashup'); ?></option>
		...
		<p id="field-cat-title-<?php echo absint($item_id); ?>" class="field-cat-title description description-wide" style="display: <?php echo ($item->view == 'cat-view-2' ? 'block' : 'none') ?>;">
			...
			<?php esc_html_e('Title', 'mashup'); ?><br />
		...
		<p id="field-item-categories-<?php echo absint($item_id); ?>" class="field-categories description description-wide" style="display: <?php echo ($item->view != 'simple' ? 'block' : 'none') ?>;">
			...
			<?php esc_html_e('Categories', 'mashup'); ?><br />
			<select id="edit-menu-item-categories-<?php echo absint($item_id); ?>" multiple class="widefat edit-menu-item-categories" name="menu-item-categories[<?php echo absint($item_id); ?>][]">
			    <option value=""><?php esc_html_e('-- Select Categories --', 'mashup'); ?></option>
			    <?php
		...
		<div class="menu-item-actions description-wide submitbox">
		    <?php if ('custom' != $item->type && $original_title !== false) : ?>
			    ...
			    <?php printf(__('Original: %s', 'mashup'), '<a href="' . esc_attr($item->url) . '">' . esc_html($original_title) . '</a>'); ?>
	    	    </p>
		    <?php endif; ?>
		    ...
		    ?>"><?php esc_html_e('Remove', 'mashup'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo absint($item_id); ?>" href="<?php echo esc_url(add_query_arg(array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg($removed_args, admin_url('nav-menus.php'))));
		    ?>#menu-item-settings-<?php echo absint($item_id); ?>"><?php esc_html_e('Cancel', 'mashup'); ?></a>
		</div>
