<?php
/**
 * Template part for displaying post medium slider format.
 *
...
if ( is_array( $gallery_images ) && array_filter( $gallery_images ) ) {
    ?>
    <div class="blog-medium-slider-holder swiper-container">
        <div class="swiper-loader"><div class="slice"><div data-loader="ball-roll"></div></div></div>
        <ul class="blog-medium-slides swiper-wrapper">
            <?php
            foreach ( $gallery_images as $key => $gallery_image_id ) {
                if ( '' != $gallery_image_id ) {
                    ...
                    echo '<li class="swiper-slide"><a href="javascript:void(0);"><img src="' . esc_url( $mashup_var_src[0] ) . '" alt="' . esc_html( $image_alt ) . '" /></a></li>';
                }
            }
            ?>
        </ul>
