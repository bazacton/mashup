 ...
 * </code>
 * Example : Note : define your Prefix (like tabs || tab_item ) in pattren. (ref. mashup_get_pattern())
 ...
 *  [/tabs]';
 * 	$Output = array();
 *  $PREFIX = 'prefix'; //user prefix as mashup_message OR mashup_tabs | tab_item
 * 	$parseInstance 	= new ShortcodeParse();
 * 	$output = $b->mashup_shortcodes( $output, $text );
 * 	echo '<pre>';
...
if (!class_exists('ShortcodeParse')) {
    ...
    class ShortcodeParse {
        ...
        function mashup_get_pattern($content, $PREFIX) {
        ...
        function mashup_parse_atts($content) {
        ...
        function mashup_shortcodes(&$output, $content, $child = true, $PREFIX) {
            $patts = $this->mashup_get_pattern($content, $PREFIX);
            $t = array_filter($this->mashup_get_pattern($content, $PREFIX));
            if (!empty($t)) {
                ...
                foreach ($parents as $k => $parent) {
                    ...
                    $t = array_filter($this->mashup_get_pattern($contents[$k], $PREFIX));
                    $t_s = $this->mashup_shortcodes($outputNew, $contents[$k], true, $PREFIX);
                    $output[$name] = array('name' => $parents[$k]);
                    $output[$name]['atts'] = $this->mashup_parse_atts($atts[$k]);
