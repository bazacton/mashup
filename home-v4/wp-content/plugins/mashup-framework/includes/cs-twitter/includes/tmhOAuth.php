...
class tmhOAuth {
    ...
    public function reconfigure($config = array()) {
        ...
        $this->config = array_merge(
            ...
            'is_streaming' => false,
            'streaming_eol' => "\r\n",
            'streaming_metrimashup_interval' => 10,
    ...
    private function update_metrics() {
        $now = time();
        if (($this->metrics['interval_start'] + $this->config['streaming_metrimashup_interval']) > $now)
            return null;

        $this->metrics['mps'] = round(($this->metrics['messages'] - $this->metrics['last_messages']) / $this->config['streaming_metrimashup_interval'], 2);
        $this->metrics['bps'] = round(($this->metrics['bytes'] - $this->metrics['last_bytes']) / $this->config['streaming_metrimashup_interval'], 2);
