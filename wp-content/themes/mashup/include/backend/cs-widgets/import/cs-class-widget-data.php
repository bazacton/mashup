...
if ( !class_exists( 'mashup_var_widget_data' ) ) {
	...
	class mashup_var_widget_data {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_mashup_widget_file_delete', array( __CLASS__, 'mashup_widget_file_delete' ) );
		}
		...
		public static function export_settings_page() {
			...
			<div class="widget-data cs-export-widget-settings">
				...
				<div class="cs-export-wrapper">
					<div id="notifier" <?php echo esc_html( $display ); ?> ></div>
					<div id="cs-widget-export-form">
						<div class="title">
							<h3><?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_widget_setting' ) ); ?></h3>
							<span class="cs-select-butns">
								<a class="select-all"><?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_select_all' ) ); ?></a>
								<a class="unselect-all"><?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_unselect_all' ) ); ?></a>
							</span>
							<div class="clear"></div>
						</div>
						<div class="sidebars">
			...
			<?php
			$mashup_output = ob_get_clean();
			return $mashup_output;
		}
				...
				<?php if ( isset( $_POST['file_name'] ) && $_POST['file_name'] != '' ) : ?>
					...
					<div class="cs-import-wrapper">
						...
						<div id="cs-import-widget-form">
							...
							<div class="title">
								<h3><?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_widget_setting_import' ) ); ?></h3>
								<span class="cs-select-butns">
									<a class="imp-select-all"><?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_select_all' ) ); ?></a>
									<a class="imp-unselect-all"><?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_unselect_all' ) ); ?></a>
								</span>
								<div class="clear"></div>
							</div>
							<div class="sidebars">
							...
							<p>
								...
								<label for="clear-current"><?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_clear_current' ) ); ?></label><br/>
								<span class="description"><?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_all_active' ) ); ?></span>
							</p>
							<input type="button" name="import-widgets" id="cs-import-wgts-btn" value="<?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_import_widget_settings' ) ); ?>" />
						</div>
					</div>
					<?php
				else :
					?>
					<p><?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_put_file' ) ); ?></p>
					<p>
						<input type="text" id="cs-widget-upload-file" />
					</p>
					<input type="button" id="widget-import-submit" class="button" value=<?php echo esc_html( mashup_var_theme_text_srt( 'mashup_var_show_widget_settings' ) ); ?> />
				<?php endif; ?>
		...
		public static function export_widget_settings() {
			...
			if ( isset( $_POST['action'] ) && $_POST['action'] == 'export_widget_settings' ) {
				$backup_url = wp_nonce_url( 'themes.php?page=mashup_options_page' );
				...
				$mashup_upload_dir = get_template_directory() . '/include/backend/cs-widgets/import/widgets-backup/';

				$mashup_filename = trailingslashit( $mashup_upload_dir ) . 'widget_' . (current_time( 'd-M-Y_H.i.s' )) . '.json';

				$mashup_widgets_file = self::parse_export_data( $_POST );

				if ( !$wp_filesystem->put_contents( $mashup_filename, $mashup_widgets_file, FS_CHMOD_FILE ) ) {
		...
		public static function mashup_widget_file_delete() {

			global $wp_filesystem, $mashup_var_static_text;
			$strings = new mashup_theme_all_strings;
			$strings->mashup_theme_option_field_strings();
			$mashup_var_file_deleted_successfully = mashup_var_theme_text_srt( 'mashup_var_file_deleted_successfully' );
			...
			$mashup_filename = trailingslashit( $mashup_upload_dir ) . $file_name;

			if ( is_file( $mashup_filename ) ) {
				unlink( $mashup_filename );
				printf( $mashup_var_file_deleted_successfully, $file_name );
			} else {
		...
		public static function ajax_import_widget_data() {
			...
			if ( empty( $wp_filesystem ) ) {
				mashup_include_file( ABSPATH . '/wp-admin/includes/file.php' );
				WP_Filesystem();
			}
		 ...
		 */
		public static function mashup_import_widget_data( $filename = "" ) {
			...
			if ( $filename == "" ) {
				$mashup_upload_dir = get_template_directory() . '/include/backend/cs-widgets/import/import-widgets/';

				$file_name = 'import-widgets.json';

				$import_file = trailingslashit( $mashup_upload_dir ) . $file_name;
			} else {
			...
			if ( is_array( $sidebar_data ) ) {
				...
				echo '<p><strong>' . esc_html( mashup_var_theme_text_srt( 'mashup_var_widget_imported' ) ) . '</strong><br />';
			} else {
				echo '<p><strong>' . esc_html( mashup_var_theme_text_srt( 'mashup_var_widget_not' ) ) . '</strong><br />';
			}
		}
...
$mashup_var_widget_data = new mashup_var_widget_data();
