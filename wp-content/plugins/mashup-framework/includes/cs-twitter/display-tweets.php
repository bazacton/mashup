...
function get_auth( $id, $max_tweets ) {
    ...
    $consumer_key = $mashup_twitter_arg['consumerkey'];
    $consumer_secret = $mashup_twitter_arg['consumersecret'];
    $user_token = $mashup_twitter_arg['accesstoken'];
    $user_secret = $mashup_twitter_arg['accesstokensecret'];
...
function display_tweets( $id, $style = '', $max_tweets = 10, $max_cache_tweets = 10, $time = 60 ) {
    ...
    if ( ! empty( $tweets ) ) {
        ...
        foreach ( $tweets as $tweet ) {
            ...
            if ( isset( $style ) ) {
                if ( ! empty( $style ) ) {
                    $when = ( 'time_since' === $style ) ? '' : esc_html( mashup_var_frame_text_srt( 'mashup_var_tweets_time_on' ) );
                    $twitter .= '<span>' . $when . '&nbsp;';
        ...
        $twitter .= '<li>' . esc_html( mashup_var_frame_text_srt( 'mashup_var_no_tweets_found' ) ) . '</li>';
    } //end if statement.
 ...
 * @param number $time tweets time.
 */
function display_tweets_element( $id, $style = '', $max_tweets = 10, $max_cache_tweets = 10, $time = 60 ) {
    global $mashup_var_frame_static_text;
    $tweets = cache_json( $id, $max_tweets, $time );
    $twitter = '';
    ...
    if ( ! empty( $tweets ) ) {
        ...
        foreach ( $tweets as $tweet ) {
            ...
            $twitter .= '<div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
								<div class="twitter-post">
									<p>' . $tweet . '</p>
									<div class="post-options">
                                                                        <strong class="text-color">BY</strong>
                                                                        <span><a target="blank" title="" href="http://twitter.com/'.$id.'">@' . $id . '</a></span>
								';
            if ( isset( $style ) ) {
                if ( ! empty( $style ) ) {
                    $when = ( 'time_since' === $style ) ? '' : esc_html( mashup_var_frame_text_srt( 'mashup_var_tweets_time_on' ) );
                    $twitter .= '<strong class="text-color">' . $when . '&nbsp;';
