...
add_action('admin_init', 'mashup_check_for_updates');
if ( ! function_exists('mashup_check_for_updates') ) {

	/**
	 * Check for theme updates if available
	 */
	function mashup_check_for_updates() {
		global $mashup_options;
		...
		if ( $upgrade_option ) {
			...
			if ( 'themes.php' == basename($_SERVER[PHP_SELF]) ) {
				add_action('admin_footer', 'mashup_show_theme_auto_update_available_on_themes_page');
			}

			add_action('admin_notices', 'mashup_show_theme_auto_update_available');
			return;
		}

		//- delete_option( 'last_time_auto_theme_update_checked_' . THEME_NAME );
		if ( ! empty($mashup_options['cs_marketplace_username']) ) {
			...
			$user_name = $mashup_options['cs_marketplace_username'];
			$api_key = $mashup_options['cs_secret_api_key'];
			$skip_backup = $mashup_options['cs_skip_theme_backup'];
			...
			if ( $errors = $protected_api->api_errors() ) {
				add_action('admin_notices', 'mashup_show_theme_auto_update_error');
			} else {
			...
			add_action('admin_notices', 'mashup_show_theme_auto_update_warning');
		}
	}
...
if ( ! function_exists('mashup_show_theme_auto_update_available_on_themes_page') ) {

	/**
	 * Show theme update available on themes page, Ouput JS to footer of themes.php page.
	 */
	function mashup_show_theme_auto_update_available_on_themes_page() {
...
if ( ! function_exists('mashup_show_theme_auto_update_available') ) {

	/**
	 * Show notification if an update is avaiable.
	 */
	function mashup_show_theme_auto_update_available() {
		$data = get_option('is_auto_update_theme_' . THEME_NAME);
		if ( $data ) {
			$class = 'notice notice-warning is-dismissible';
			$plugins = mashup_get_plugins_to_be_updated();
...
if ( ! function_exists('mashup_show_theme_auto_update_error') ) {

	/**
	 * Show error if marketplace username or secret API keys is not valid.
	 */
	function mashup_show_theme_auto_update_error() {
		$class = 'notice notice-error is-dismissible';
		$message = __('Please provide valid Marketplace Username and Secret API Key in theme options to get Automatic Theme Updates.  &raquo; <a href="' . network_admin_url('admin.php?page=' . THEME_OPTIONS_PAGE_SLUG) . '#tab-auto-updater-show">Fix Now</a>', 'mashup');
...
if ( ! function_exists('mashup_show_theme_auto_update_warning') ) {

	/**
	 * Show Warning if auto updates are not configured.
	 */
	function mashup_show_theme_auto_update_warning() {
		if ( false === ( $admin_dismiss_notice = get_transient('admin_dismiss_notice') ) ) {
			$class = 'notice notice-warning is-dismissible';
			$message = __('Please configure Auto Updates in theme options to get Automatic Theme Updates. &raquo; <a href="' . network_admin_url('admin.php?page=' . THEME_OPTIONS_PAGE_SLUG) . '#tab-auto-updater-show">Configure Now</a>', 'mashup');
...
if ( ! function_exists('mashup_auto_upgrade_theme_and_plugins') ) {
	 ...
	 */
	function mashup_auto_upgrade_theme_and_plugins() {
		global $mashup_options, $current_screen, $cs_theme_upgraded_name, $cs_plugins_upgraded;
		...
		// Upgrade Theme.
		$theme_info['user_name'] = $mashup_options['cs_marketplace_username'];
		$theme_info['api_key'] = $mashup_options['cs_secret_api_key'];
		$theme_info['skip_backup'] = $mashup_options['cs_skip_theme_backup'];
		$status = true;
		$status = mashup_auto_update_theme($theme_info);

		// Upgrade Plugins.
		if ( function_exists('mashup_get_plugins_to_be_updated') ) {
			mashup_auto_update_plugins(mashup_get_plugins_to_be_updated());
		}
...
if ( ! function_exists('mashup_auto_update_theme') ) {
	 ...
	 */
	function mashup_auto_update_theme($theme_info) {
...
if ( ! function_exists('mashup_auto_update_plugins') ) {
	 ...
	 */
	function mashup_auto_update_plugins($plugins) {
		...
		foreach ( $plugins as $key => $plugin_info ) {
			if ( '' != $plugin_info['slug'] ) {
				...
				mashup_include_file(ABSPATH . 'wp-admin/includes/class-wp-upgrader.php'); // Need for upgrade classes.
...
if ( ! function_exists('mashup_backup_theme') ) {
	 ...
	 */
	function mashup_backup_theme($theme) {
		$backup_errors = array();
