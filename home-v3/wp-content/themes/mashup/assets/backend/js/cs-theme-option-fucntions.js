function mashup_theme_option_save(admin_url, theme_url) {
    "use strict";
...
function mashup_var_set_filename(file_value, file_path) {
    "use strict";
...
function mashup_var_backup_generate(admin_url) {
    "use strict";
    jQuery(".outerwrapp-layer,.loading_div").fadeIn(100);

    var dataString = 'action=mashup_var_settings_backup_generate';
    jQuery.ajax({
        type: "POST",
        url: admin_url,
        data: dataString,
...
jQuery('.backup_generates_area').on('click', '#cs-backup-delte', function () {
    ...
    if (var_confirm == true) {
        ...
        var dataString = 'file_name=' + file_name + '&action=mashup_var_backup_file_delete';
        jQuery.ajax({
            type: "POST",
            url: admin_url,
            data: dataString,
...
jQuery('.backup_generates_area').on('click', '#cs-backup-restore, #cs-backup-url-restore', function () {
    ...
    var dataString = 'file_name=' + file_name + '&action=mashup_var_backup_file_restore';

    if (typeof (file_name) === 'undefined') {

        var file_name = jQuery('#bkup_import_url').val();

        var dataString = 'file_name=' + file_name + '&file_path=yes&action=mashup_var_backup_file_restore';
    }
...
function mashup_var_remove_image(id) {
    "use strict";
    var $ = jQuery;
    $('#' + id).val('');
    $('#' + id + '_img_div').hide();
...
function mashup_var_banner_type_toggle(type, id) {
    "use strict";
    if (type == 'image') {
...
function mashup_var_google_font_att(admin_url, att_id, id) {
    "use strict";
    ...
    if (att_id != "") {
        ...
        var dataString = 'index=' + att_id + '&id=' + id +
                '&action=mashup_var_get_google_font_attributes';
        jQuery.ajax({
            type: "POST",
            url: admin_url,
            data: dataString,
...
function  mashup_var_add_banner(admin_url) {
    "use strict";
    counter_banner++;
    var image_path = jQuery('#mashup_var_banner_field_image').val();
    ...
    if (banner_style_input != "") {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                jQuery("#mashup_var_banner_field_image,#banner_title_input,#banner_field_url_input,#adsense_code_input").val("");
...
function mashup_var_add_social_icon(admin_url) {
    "use strict";
    counter_social_network++;
    var social_net_icon_path = jQuery("#mashup_var_social_icon_input").val();
    ...
    if (social_net_url != "" && (social_net_icon_path != "" || social_net_awesome != "")) {
        var dataString = 'social_net_icon_path=' + social_net_icon_path +
                ...
                '&action=mashup_var_add_social_icon';
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                jQuery(".social-area").show(200);
                jQuery("#mashup_var_social_icon_input,#social_net_awesome_input,#social_net_url_input,#social_net_tooltip_input").val("");
...
function select_bg(layout, value, theme_url, admin_url) {
    ...
    if (value == 'boxed' && layout == 'mashup_layout') {
        jQuery('.horizontal_tabs,.main_tab').show();
    } else if (value == 'full_width' && layout == 'mashup_layout') {
        jQuery('.horizontal_tabs,.main_tab').hide();
        jQuery('#mashup_bg_color').hide();
...
function mashup_var_div_remove(id) {
    "use strict";
    jQuery("#" + id).remove();
}
...
function add_sidebar() {
    ...
    if (sidebar_input != "") {
        jQuery("#sidebar_area").append('<tr id="' + counter_sidebar + '"> \
                            <td><input type="hidden" name="mashup_var_sidebar[]" value="' + sidebar_input + '" />' + sidebar_input + '</td> \
                            <td class="centr"> <a class="remove-btn" onclick="javascript:return confirm(\'Are you sure! You want to delete this\')" href="javascript:mashup_var_div_remove(' + counter_sidebar + ')"><i class="icon-cancel"></i></a> </td> \
                        </tr>');
...
function add_footer_sidebar() {
    ...
    if (footer_sidebar_input != "" || footer_sidebar_width != "") {
        jQuery("#footer_sidebar_area").append('<tr id="' + counter_footer_sidebar + '"> \
									<td><input type="hidden" name="mashup_var_footer_sidebar[]" value="' + footer_sidebar_input + '" />' + footer_sidebar_input + '</td> \
									<td><input type="hidden" name="mashup_var_footer_width[]" value="' + footer_sidebar_width + '" />' + footer_sidebar_width + '</td> \
									<td class="centr"> <a class="remove-btn" onclick="javascript:return confirm(\'Are you sure! You want to delete this\')" href="javascript:mashup_div_remove(' + counter_footer_sidebar + ')"><i class="icon-cancel"></i></a> </td> \
								</tr>');
...
function mashup_var_set_headerbg(value) {
    "use strict";

    if (value == 'absolute') {

        jQuery('#mashup_var_headerbg_options_header,#mashup_var_headerbg_image_upload,#mashup_var_headerbg_color_color,#mashup_var_headerbg_slider_1,#mashup_var_headerbg_image_box').show();
        if (jQuery('#mashup_var_headerbg_options').val() == 'mashup_var_rev_slider') {
            jQuery('#mashup_var_headerbg_slider_1').show();
            jQuery('#mashup_var_headerbg_image_upload,#mashup_var_headerbg_color_color,#tab-header-options #mashup_var_headerbg_image_box').hide();
        } else if (jQuery('#mashup_var_headerbg_options').val() == 'mashup_var_bg_image_color') {
            jQuery('#mashup_var_headerbg_slider_1').hide();
            jQuery('#mashup_var_headerbg_image_upload,#mashup_var_headerbg_color_color,#tab-header-options #mashup_var_headerbg_image_box').show();
        } else {
            jQuery('#mashup_var_headerbg_slider_1').hide();
            jQuery('#mashup_var_headerbg_image_upload,#mashup_var_headerbg_color_color,#tab-header-options #mashup_var_headerbg_image_box,#mashup_var_headerbg_slider_1').hide();
        }

    } else if (value == 'relative') {

        jQuery('#mashup_var_headerbg_options_header,#mashup_var_headerbg_image_upload,#mashup_var_headerbg_color_color,#mashup_var_headerbg_slider_1,#tab-header-options #mashup_var_headerbg_image_box').hide();

    } else if (value == 'mashup_var_rev_slider') {

        jQuery('#mashup_var_headerbg_slider_1').show();

        jQuery('#mashup_var_headerbg_image_upload,#mashup_var_headerbg_color_color,#tab-header-options #mashup_var_headerbg_image_box').hide();

    } else if (value == 'mashup_var_bg_image_color') {
