			...
			<div class="menu-item-bar">
				<div class="menu-item-handle">
					...
					<span class="item-controls">
						...
						<a class="item-edit" id="edit-<?php echo absint($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item'); ?>" href="<?php
				...
				<p class="field-view description description-wide">
					<label for="edit-menu-item-view-<?php echo absint($item_id); ?>">
						...
						<select id="edit-menu-item-view-<?php echo absint($item_id); ?>" onchange="mashup_menu_view_select(this.value, '<?php echo absint($item_id); ?>')" class="widefat edit-menu-item-view" name="menu-item-view[<?php echo absint($item_id); ?>]">
							<option<?php echo esc_attr($item->view) == 'simple' ? ' selected="selected"' : '' ?> value="simple"><?php _e('Simple', 'mashup'); ?></option>
							<option<?php echo esc_attr($item->view) == 'cat-view-1' ? ' selected="selected"' : '' ?> value="cat-view-1"><?php _e('Categories View 1', 'mashup'); ?></option>
							<option<?php echo esc_attr($item->view) == 'cat-view-2' ? ' selected="selected"' : '' ?> value="cat-view-2"><?php _e('Categories View 2', 'mashup'); ?></option>
							<option<?php echo esc_attr($item->view) == 'cat-view-3' ? ' selected="selected"' : '' ?> value="cat-view-3"><?php _e('Categories View 3', 'mashup'); ?></option>
						</select>
					</label>
				</p>
				...
				<p id="field-item-categories-<?php echo absint($item_id); ?>" class="field-categories description description-wide" style="display: <?php echo ($item->view != 'simple' ? 'block' : 'none') ?>;">
					<label for="edit-menu-item-categories-<?php echo absint($item_id); ?>">
						...
						<select id="edit-menu-item-categories-<?php echo absint($item_id); ?>" multiple class="widefat edit-menu-item-categories" name="menu-item-categories[<?php echo absint($item_id); ?>][]">
							<option value=""><?php _e('-- Select Categories --', 'mashup'); ?></option>
							<?php
...
<!--				<p id="field-item-view-all-<?php echo absint($item_id); ?>" class="field-view-all description description-wide" style="display: <?php echo ($item->view == 'cat-view-3' ? 'block' : 'none') ?>;">
					<label for="edit-menu-item-view-all-<?php echo absint($item_id); ?>">
						<?php _e('View All Link'); ?><br />
						<input type="text" id="edit-menu-item-view-all-<?php echo absint($item_id); ?>" class="widefat code edit-menu-item-view-all" name="menu-item-view-all[<?php echo absint($item_id); ?>]" value="<?php echo esc_attr($item->view_all_link); ?>" />
					</label>
				</p>-->
				<?php
				...
				<div class="menu-item-actions description-wide submitbox">
					...
					?>"><?php _e('Remove'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo absint($item_id); ?>" href="<?php echo esc_url(add_query_arg(array( 'edit-menu-item' => $item_id, 'cancel' => time() ), remove_query_arg($removed_args, admin_url('nav-menus.php'))));
					?>#menu-item-settings-<?php echo absint($item_id); ?>"><?php _e('Cancel'); ?></a>
				</div>
